...
var Flow = function( config, igneous ){
	...
	config.minify = ( typeof config.minify !== 'undefined' )? config.minify: igneous.config.minify;
	config.watch = ( typeof config.watch !== 'undefined' )? config.watch: igneous.config.watch;
	config.encoding = config.encoding || igneous.config.encoding;
	...
	flow.url = null;
	flow.modified = null;
	flow.store = igneous.config.store;
	...
	flow.add = function(){
		...
		var base = path.join( igneous.config.root, config.base );
	...
	flow.watch = function(){
		...
		var file_paths = config.paths.map( function( file_path ){
			...
			file_path = path.join( igneous.config.root, config.base, file_path );
