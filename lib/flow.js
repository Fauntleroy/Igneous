...
var Flow = function( config, igneous_config ){
	...
	} else if( flow.type === 'sass' ){
		config.postprocessors.unshift('sass');
	} else if( flow.type === 'less' ){
		config.postprocessors.unshift('less');
	}
	...
	flow.preprocess = function( cb ){
		...
		async.forEach( files_array, function( file, cb ){
			async.forEachSeries( config.preprocessors, function( preprocessor, callback ){
