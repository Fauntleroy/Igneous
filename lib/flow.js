...
console.log( 'preprocess', flow.type );
	if( flow.type === 'jst' ){
		if( flow.config.jst_lang === 'handlebars' ){
			data = preprocessors.handlebars( path, data, flow );
		} else if( flow.config.jst_lang === 'jquery-tmpl' ){
			data = preprocessors.jqtpl( path, data, flow );
		}
	}
...
var Flow = function( config, igneous_config ){
	...
	config.compress = ( typeof config.compress !== 'undefined' )? config.compress: igneous_config.compress;
	...
	flow.flow = function(){
		...
		igneous_config.host.store( flow, function( flow ){
			console.log('stored');
			//igneous[config.route.toString()] = flow.url;
			//flow.data = null;
