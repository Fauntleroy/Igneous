...
var _getTruePath = function( root, path ){
	...
	if( root_trailing_slash && ( path === '/' || path === '\\' ) )
		...
		true_path = ( root_trailing_slash || root === '' )
			? root + path
			: root +'/'+ path;
	}
...
igneous.Flow = function( attributes ){
	...
	flow._add = function(){
		...
		var addFiles = function( paths, root ){
			...
			paths.forEach( function( path, i ){	
				...
				if( !exists )
					return console.error('WARNING: '+ true_path +' does not exist!', __dirname, root, path);
	...
	flow._send = function(){
		...
		var request = s3.put( '/'+ time +'_'+ flow.name, {
			...
			'Content-Type': CONTENT_TYPES[flow.type]
		});

		request.on( 'response', flow._receive );
	...
	flow._receive = function( res ){

		if( res.statusCode !== 200 )
			throw new Error( 'S3 upload failed' );
		else {
			igneous[flow.name] = flow.url = res.client._httpMessage.url;
			flow.string = '';
		}
